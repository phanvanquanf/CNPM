...
document.addEventListener("DOMContentLoaded", function () {
        ...
        // Xử lý khi chọn ảnh
        document.querySelectorAll('#addForm input[type="file"]').forEach(input => {
            input.addEventListener('change', e => {
                const file = e.target.files[0];
                if (!file) return;

                const id = e.target.id;
                const masophong = document.getElementById('create-masophong').value.trim() || 'temp';
                ...
                if (id.includes('create-anhphu')) {
                    ...
                    document.getElementById(`create-anh-link-${index}`).textContent = fullPath;
        ...
        function bindEditButtons() {
            ...
            modal.addEventListener('show.bs.modal', event => {
                ...
                const fixPath = p => p ? (p.includes("assets/img/") ? p : baseFolder + p) : "/assets/img/no-image.png";
            ...
            document.querySelectorAll('#editForm input[type="file"]').forEach(input => {
                input.addEventListener('change', e => {
                    const file = e.target.files[0];
                    if (!file) return;
            ...
            $(document).off('submit', '#editForm').on('submit', '#editForm', function (e) {
                ...
                if (masophong === '') { $('#error-MaSoPhong').text('Mã số phòng không được để trống'); valid = false; }
                if ($('#edit-vitri').val().trim() === '') { $('#error-ViTri').text('Vị trí không được để trống'); valid = false; }
                if ($('#edit-giaphong').val().trim() === '') { $('#error-GiaPhong').text('Giá phòng không được để trống'); valid = false; }
                if ($('#edit-tongquan').val().trim() === '') { $('#error-TongQuan').text('Tổng quan không được để trống'); valid = false; }
                if ($('#edit-idloaiphong').val() === '0') { $('#error-IDLoaiPhong').text('Phải chọn loại phòng'); valid = false; }
                if ($('#edit-trangthai').val() === '') { $('#error-TrangThai').text('Phải chọn trạng thái phòng'); valid = false; }
                if (!valid) return;
